...
var linkTo;
var exectute = (function(nodesTodisplay) {
    const allNodes = [
    ...
    if(!nodesTodisplay){
       ...
       nodes = allNodes.filter(el => {
           return nodesTodisplay.includes(el.id);

            });

        links = allLinks.filter(el => {

          return nodesTodisplay.includes(el.target) && nodesTodisplay.includes(el.source);
    ...
    function filterNodesAndLinks(nodesIdToDisplay){
